...
def method_ShurPartialJacobi(A, B, C, D, f1, f2, nit, toll, type):
    ...
    for i in range(nit):
        nitfinal += 1
        if type=='alternate':
            u = sparse.linalg.spsolve(A - B@invDD@C, f1 - B@invDD@(f2 + (DD-D)@v))
            v = sparse.linalg.spsolve(D - C@invAA@B, f2 - C@invAA@(f1 + (AA-A)@u))
        if type=='shur_on_u':
            u = sparse.linalg.spsolve(A - B@invDD@C, f1 - B@invDD@(f2 + (DD-D)@v))
            v = sparse.linalg.spsolve(            D, f2 - C@u)
        if type=='shur_on_v':
...
def method_SPJ_stab(A, B, C, D, f1, f2, nit, toll, type='alternate'):
    nitfinal = 0
    AA    = sparse.csc_matrix(np.diag(A.diagonal(), 0))
    invAA = sparse.csc_matrix(np.diag(1/A.diagonal(), 0))
    DD    = sparse.csc_matrix(np.diag(D.diagonal(), 0))
...
def method_ShurDualPartialJacobi(A, B, C, D, f1, f2, nit, toll, type='alternate'):
    nitfinal = 0
    AA    = sparse.csc_matrix(np.diag(A.diagonal(), 0))
    invAA = sparse.csc_matrix(np.diag(1/A.diagonal(), 0))
...
def method_Lscheme(A, B, C, D, f1, f2, nit, toll, type, Lu, Lv):
    nitfinal = 0
    u = np.zeros(A.shape[0])
    v = np.zeros(A.shape[0])
    res = []
