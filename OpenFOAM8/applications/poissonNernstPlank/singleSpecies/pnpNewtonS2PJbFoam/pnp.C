...
int main(int argc, char *argv[])
{
    ...
    while (runTime.loop())
    {
        ...
        while ( pimple.loop() ) // coupling loop
        {
            ...
            fvScalarMatrix Aeqn( fvm::ddt(v) - fvm::div(fvc::flux(z2*fvc::grad(u)), v) - fvm::laplacian(a, v) );
            fvScalarMatrix Ceqn( fvm::Sp(z1, v) );

            volScalarField CinvA(Ceqn.A()*(scalar(1)/Aeqn.A()));

            u.storePrevIter();

            solve( fvm::laplacian(m, u) - fvm::laplacian(-CinvA*z2*v, u) == Ceqn.H() - CinvA*(-fvc::div(fvc::flux(z2*v*fvc::grad(u))) + Aeqn.H()) );

            // solve( fvm::ddt(v) - fvm::div(fvc::flux(z2*fvc::grad(u.prevIter())), v) - fvm::laplacian(a, v) == fvc::div(fvc::flux(z2*v*fvc::grad(u-u.prevIter()))));
            solve( fvm::ddt(v) - fvm::div(fvc::flux(z2*fvc::grad(u)), v) - fvm::laplacian(a, v) == fvc::div(fvc::flux(z2*v*fvc::grad(u-u.prevIter()))));
