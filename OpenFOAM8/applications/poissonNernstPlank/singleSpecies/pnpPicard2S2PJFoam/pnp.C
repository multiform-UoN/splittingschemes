...
int main(int argc, char *argv[])
{
    ...
    while (runTime.loop())
    {
        ...
        while ( pimple.loop() ) // coupling loop
        {
            ...
            fvScalarMatrix Aeqn( fvm::ddt(v) - fvm::laplacian(a, v) );

            fvScalarMatrix Ceqn( fvm::laplacian(z1, v) );

            volScalarField CinvA(Ceqn.A()*(scalar(1)/Aeqn.A()));

            solve( fvm::laplacian(m, u) + fvm::laplacian(CinvA*z2*v, u) == Ceqn.H() - CinvA*Aeqn.H() );

            solve( fvm::ddt(v) - fvm::laplacian(a, v, "laplacian(a,v)") == fvc::div(fvc::flux(z2*v*fvc::grad(u))) );
